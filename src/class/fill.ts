...
export class Fill {
    ...
    constructor(canvas, point, color) {
        ...
        this.getImageData = this.context.getImageData(0, 0, this.context.canvas.width, this.context.canvas.height)
    ...
    setPixel(point, fillColor) {
        const offset = (point.y * this.getImageData.width + point.x) * 4;
        this.getImageData.data[offset + 0] = fillColor[0];
        this.getImageData.data[offset + 1] = fillColor[1];
        this.getImageData.data[offset + 2] = fillColor[2];
        this.getImageData.data[offset + 3] = fillColor[3];
    }

    getPixel(point) {
        if (point.x < 0 || point.y < 0 || point.x >= this.getImageData.width, point.y >= this.getImageData.height) {
            return [-1, -1, -1, -1];
        } else {
            const offset = (point.y * this.getImageData.width + point.x) * 4;
            return [
                this.getImageData.data[offset + 0],
                this.getImageData.data[offset + 1],
                this.getImageData.data[offset + 2],
                this.getImageData.data[offset + 3]
            ]
        }
    }
